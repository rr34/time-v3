...
const ClockStrings: React.FC<ClockStringsProps> = ({ CurrentTime, SunIndex, SunDaily, MoonDaily, NearestNew, NearestNewAngle, NearestFull, NearestFullAngle }) => {
    ...
    if (Math.abs(timedelta_new) < Math.abs(timedelta_full) && Math.sign(timedelta_new) > 0) {
        ...
        let eclipse_string: string = (NearestNewAngle > 178.5) ? ' > ~~178.5° means solar eclipse.' : '';
        moonphasestring = timedelta_str + ' since new moon at ' + NearestNewAngle.toString() + '° phase angle.' + eclipse_string;
    }
    else if (Math.abs(timedelta_new) < Math.abs(timedelta_full) && Math.sign(timedelta_new) < 0) {
        let timedelta_str: string = msToTime(Math.abs(timedelta_new));
        let eclipse_string: string = (NearestNewAngle > 178.5) ? ' > than ~~178.5° means solar eclipse.' : '';
        moonphasestring = timedelta_str + ' until new moon at ' + NearestNewAngle.toString() + '° phase angle.' + eclipse_string;
    }
        ...
        let eclipse_string: string = (NearestFullAngle < 1.5) ? ' < ~1.5° means lunar eclipse.' : '';
        moonphasestring = timedelta_str + ' since full moon at ' + NearestFullAngle.toString() + '° phase angle.' + eclipse_string;
    }
    else if (Math.abs(timedelta_new) > Math.abs(timedelta_full) && Math.sign(timedelta_full) < 0) {
        let timedelta_str: string = msToTime(Math.abs(timedelta_full));
        let eclipse_string: string = (NearestFullAngle < 1.5) ? ' < ~1.5° means lunar eclipse.' : '';
        moonphasestring = timedelta_str + ' until full moon at ' + NearestFullAngle.toString() + '° phase angle.' + eclipse_string;
    }
    ...
    industrialdtstring = ('Industrial Time: ' + new Intl.DateTimeFormat("en-GB", options).format(CurrentTime));
